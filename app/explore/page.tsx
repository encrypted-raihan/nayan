...
const layers: Layer[] = [
  ...
  { id: "events", label: "Natural Events", detail: "INDIA + SURROUNDING REGION", status: "live" },
...
export default function ExplorePage() {
  ...
  useEffect(() => {
    const onEarthquakeSelected = (event: Event) => {
      setSelectedNaturalEvent(null);
      setSelectedEarthquake((event as CustomEvent<EarthquakeSelection>).detail);
    };
    const onNaturalEventSelected = (event: Event) => {
      setSelectedEarthquake(null);
      setSelectedNaturalEvent((event as CustomEvent<NaturalEventSelection>).detail);
    };
    const onEarthquakeDeselected = () => setSelectedEarthquake(null);
    const onNaturalEventDeselected = () => setSelectedNaturalEvent(null);
    const onEarthquakeLoaded = (event: Event) => {
      setEarthquakeCount((event as CustomEvent<{ count: number }>).detail.count);
      setNotice("Earthquake layer updated");
    };
    const onNaturalEventsLoaded = (event: Event) => {
      setNaturalEventCount((event as CustomEvent<{ count: number }>).detail.count);
      setNotice("Natural events layer updated");
    };
  ...
  const toggleLayer = (id: string) => {
    ...
    if (id === "earthquakes") {
      ...
      window.dispatchEvent(new CustomEvent("nayan:natural-events-toggle", { detail: enabling }));
    }
  };
  ...
  return (
    <main className="explore-page">
      ...
      <header className="explore-header">
        ...
        <div className="explore-context"><span className="live-dot" /><span>INDIA · FROM ABOVE</span></div>
      </header>
      ...
      {selectedEarthquake && (
        <aside className="earthquake-detail earthquake-detail--anchored" style={{
          left: selectedEarthquake.x > window.innerWidth / 2 ? Math.max(16, selectedEarthquake.x - 356) : Math.min(Math.max(16, window.innerWidth - 356), selectedEarthquake.x + 18),
          top: selectedEarthquake.y > window.innerHeight / 2 ? Math.max(16, selectedEarthquake.y - 292) : Math.min(Math.max(16, window.innerHeight - 292), selectedEarthquake.y + 18),
          right: "auto", bottom: "auto", transform: "none", width: "min(340px, calc(100vw - 32px))",
        }}>
          <div className="earthquake-detail-top">
            <div><span className="hub-eyebrow">NAYAN / EARTHQUAKE</span><div className="earthquake-magnitude">M{selectedEarthquake.earthquake.magnitude.toFixed(1)}</div></div>
            <button className="earthquake-detail-close" onClick={() => setSelectedEarthquake(null)} aria-label="Close earthquake details">×</button>
          </div>
          ...
          <div className="earthquake-meta-grid">
            ...
            <div><span>TIME</span><strong>{formatUtc(selectedEarthquake.earthquake.time)}</strong></div>
      ...
      {selected && (
        <aside className="earthquake-detail earthquake-detail--anchored" style={{
          left: selected.x > window.innerWidth / 2 ? Math.max(16, selected.x - 356) : Math.min(Math.max(16, window.innerWidth - 356), selected.x + 18),
          top: selected.y > window.innerHeight / 2 ? Math.max(16, selected.y - 292) : Math.min(Math.max(16, window.innerHeight - 292), selected.y + 18),
          right: "auto", bottom: "auto", transform: "none", width: "min(340px, calc(100vw - 32px))",
        }}>
          <div className="earthquake-detail-top">
            <div><span className="hub-eyebrow">NAYAN / NATURAL EVENT</span><div className="earthquake-magnitude">{formatNaturalCategory(selected.event)}</div></div>
            <button className="earthquake-detail-close" onClick={() => setSelectedNaturalEvent(null)} aria-label="Close natural event details">×</button>
          </div>
          <div className="earthquake-place">{selected.event.title}</div>
          <div className="earthquake-meta-grid">
            <div><span>CATEGORY</span><strong>{selected.event.categoryLabel}</strong></div>
            <div><span>DATE</span><strong>{formatUtc(selected.event.date)}</strong></div>
            <div><span>STATUS</span><strong>{selected.event.closed ? "CLOSED" : "OPEN"}</strong></div>
            <div><span>MAGNITUDE</span><strong>{selected.event.magnitude.value !== null ? `${selected.event.magnitude.value} ${selected.event.magnitude.unit ?? ""}`.trim() : "—"}</strong></div>
          </div>
          {selected.event.description && <p className="natural-event-description">{selected.event.description}</p>}
          <a className="earthquake-source" href={selected.event.sourceUrl} target="_blank" rel="noreferrer">OPEN EONET EVENT <span>↗</span></a>
        </aside>
      )}

      <button className={`hub-trigger ${open ? "is-open" : ""}`} onClick={() => setOpen((value) => !value)} aria-expanded={open}>
        <span className="hub-trigger-mark"><i /><i /><i /></span><span>CONTROL HUB</span>
      </button>

      <aside className={`control-hub ${open ? "is-visible" : ""}`} aria-hidden={!open}>
        <div className="hub-header">
          <div><span className="hub-eyebrow">NAYAN / SYSTEM</span><h2>Control Hub</h2></div>
        ...
        <section className="hub-section">
          ...
          <div className="layer-list">
            {layers.map((layer) => {
              ...
              return (
                <button key={layer.id} className={`layer-row ${active ? "is-active" : ""}`} onClick={() => toggleLayer(layer.id)}>
                  <span className="layer-state"><i /></span>
                  <span className="layer-copy"><strong>{layer.label}</strong><small>{layer.detail}</small></span>
                  <span className="layer-status">{layer.status === "live" ? "LIVE" : "DEMO"}</span>
                </button>
              );
            })}
          </div>
        ...
        <section className="hub-section hub-section--compact">
          ...
          <div className="segmented-control">
            {["Satellite", "Terrain"].map((mode) => (
              <button key={mode} className={mapMode === mode ? "is-selected" : ""} onClick={() => { setMapMode(mode); setNotice(`${mode} mode selected — demo control for now.`); }}>{mode}</button>
            ))}
          </div>
        </section>
        ...
        <section className="hub-section hub-section--compact">
          ...
          <button className="reset-view" onClick={resetIndia}><span>Reset to India</span><span>↗</span></button>
        </section>
